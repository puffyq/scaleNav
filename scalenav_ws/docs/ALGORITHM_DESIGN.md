...
| 设计对象 | 实现位置 |
|---|---|
| ROS2 在线调度 | `src/global_graph/scalenav_graph_ros2/src/scale_manager/src/epic_graph_node.cpp` |
...
| 输出 | `/epic/path` | accepted witness path | 规划成功时，默认 10 Hz |
| 输出 | `/epic/local_goal` | YOPO 当前执行点 | 规划成功时，默认 10 Hz |
| 输出 | `/epic/graph`、`/epic/bubbles` | 拓扑与自由空间可视化 | 规划/重建周期 |
| 输出 | `/epic/clearance` | 无人机及路线安全空间 | 规划周期 |

表内频率为缺省配置，实际值由 `epic_graph.launch.py` 参数确定。
...
sequenceDiagram
    ...
    participant Node as EpicGraphNode
...
| 输出 | 带语义属性的 TopoNode、局部风险查询、语义重评估请求 |
| 持久状态 | 节点语义分数、置信度、观测次数、时间戳、节点外恢复记录 |
| 异常处理 | 不支持的图像编码、非有限分数、过期帧或无匹配姿态时不更新图 |
...
flowchart LR
    ...
    Pool --> Base[低分位背景估计]
    Base --> Cal[风险校准]
    Cal --> Proj[固定 optical Z=30 m 投影]
    Proj --> World[相机FLU到世界坐标]
    World --> Track[跨帧射线关联]
    Track --> Merge[空间匹配persistent node]
    Merge --> EMA[更新score/confidence]
    EMA --> Field[对witness施加高斯风险场]
    Field --> Replan[按增量与高风险锁存请求重评估]
```
...
跨帧处理先按相机射线、无人机位移和空间重叠区关联观测，再执行 persistent node 空间匹配。无人机移动导致固定深度投影点平移时，重叠视场内的对应射线更新已有节点；只有投影足迹进入未观测空间时才新增节点。节点数量不得按“帧数 x patch 数”线性增长。

`semantic_patch_rows=3` 的三条垂向射线保留各自的世界坐标和分数。`graph_fixed_layer` 仅约束路径搜索、odom 查询点和控制目标的层高，不修改语义观测的三维坐标；需要计算二维路线风险时使用临时平面投影，不回写语义节点中心。风险锚点要求分数和置信度越过噪声门槛，分数使用 EMA 更新。A* 和路线评估只查询无人机局部搜索半径加影响带内的语义节点。
...
`31.5 m`、任务方向和 FOV 位置均只产生连续 loss。只有断连、碰撞、安全空间不足、无法生成有效 witness 或无法生成 local goal 才是 hard reject。物理可行 candidate 全部进入同一排序，较短但安全的 candidate 可以胜过更远但高风险的 candidate。`frontierCandidateLoss` 必须输出总 loss 和全部分项，供诊断及测试比较。
...
| 输出 | `/epic/path`、`/epic/local_goal`、图/气泡 marker、clearance、运行统计 |
...
| `EpicGraphNode::onFreeRays` | 自由射线消息 | 当前实现无地图占据副作用 | 不得把自由射线端点写入 M1 跨帧占据缓存或 M2 持久图 | 约 10 Hz |
...
| `TopoGraph::copyPersistentNodesFrom` | 旧图 | 新图获得跨 rebuild 的节点、边、witness 和语义记忆 | 不复制临时 odom 节点或 M1 局部点云 | 每次 detached rebuild |
...
| `EpicGraphNode::onSemanticHeatmap` | mono8/32FC1 heatmap | 时间对齐后的世界风险点帧 | patch max-pool、基线校准、姿态转换 | 约 2 Hz |
| `TopoGraph::insertSemanticNodes` | 世界点、分数、半径、观测原点、时间 | 插入/更新数量 | 近点合并、持久 id、连接仍需碰撞检查 | 每有效语义帧 |
...
| `EpicGraphNode::mergeSemanticMemory` | 图语义记录 | 节点外全局记忆 | 新时间覆盖旧时间 | 重建/语义周期 |
| `EpicGraphNode::semanticMemorySnapshot` | 无 | 节点级记忆副本 | 线程安全 | 重建周期 |
| `EpicGraphNode::semanticRiskAlongRoute` | 图、witness 折线 | `[0,1]` 路线最大风险 | 查询范围为局部半径加影响带 | 规划/语义周期 |
| `EpicGraphNode::updateTopoSemanticMemory` | 当前图、最新 SemanticFrame、无人机运动 | 应用最新帧并可能锁存 replan 请求 | 跨帧重叠观测复用 id；保留 patch 行的三维坐标；过期帧忽略；请求不清空 incumbent | 规划周期 |
...
| `EpicGraphNode::nearestPersistentNode` | 图、位置、可选 id、最大距离 | 匹配节点 | persistent id 优先，距离后备 | graph swap/规划 |
| `EpicGraphNode::ensureOdomConnectivity` | 图、路径候选容器 | 新增连接数 | odom 必须连到碰撞安全邻居 | 规划周期 |
| `EpicGraphNode::buildRememberedEdges` | forward witness | 旧路线边集合及数量 | 不记忆无人机身后边 | 规划周期 |
| `EpicGraphNode::selectNextGoal` | accepted path、found、lookahead | local goal 与成功标志 | 沿前向路径插值，固定层检查 | 成功规划周期 |

### 4.6 M6 调度与接口函数

| 函数 | 输入 | 输出/副作用 | 设计要点 | 调用频率 |
|---|---|---|---|---|
| `EpicGraphNode::configureMapBounds` | 地图、中心、margin | 初始化任务/地图边界 | 边界覆盖中心并留足 margin | 首目标/首帧 |
| `EpicGraphNode::expandMapBounds` | 地图、新点、margin | 是否扩展及新边界 | 只扩不缩，已有点云保留 | 新目标/点云 |
| `EpicGraphNode::onGoal` | PoseStamped | mission goal 和规划状态 | 新目标不留下旧目标终点；按参数复用图 | 事件触发 |
| `EpicGraphNode::startSkeletonRebuild` | 当前地图/旧图快照 | 启动一次后台 rebuild | 防重入，完成后原子交换 | 默认约 10 Hz |
| `EpicGraphNode::update` | 当前所有状态 | 一次完整规划状态迁移 | 安全 incumbent 优先；候选通过门槛才提交；连续 frontier 必须保持任务活性 | 默认 10 Hz |
| `EpicGraphNode::publish` | 图、路径节点、extension、路线状态 | ROS path/marker/goal/clearance 和统计 | 阻塞 witness 不发布 local goal | 规划周期 |
