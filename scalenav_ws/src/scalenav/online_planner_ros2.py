...
class OnlinePlanner(Node):
    ...
    def __init__(self, args: argparse.Namespace) -> None:
        super().__init__("scalenav_online_planner")
        self.args = args
        ...
        self.event_log_enabled = not args.disable_event_log
        self.event_log_dir = None
        self.event_log_path = None
        self.depth_log_dir = None
        if self.event_log_enabled:
            project_root = Path(__file__).resolve().parents[1]
            self.event_log_dir = Path(
                args.event_log_dir
                or os.environ.get("OPENSEEK_EVENT_LOG_DIR", project_root / "log_event")
            ).expanduser().resolve()
            self.event_log_dir.mkdir(parents=True, exist_ok=True)
            run_id = time.strftime("%Y%m%d_%H%M%S") + f"_{os.getpid()}"
            self.event_log_path = self.event_log_dir / f"scalenav_events_{run_id}.jsonl"
            self.depth_log_dir = self.event_log_dir / f"scalenav_depth_{run_id}"
            self.depth_log_dir.mkdir(parents=True, exist_ok=True)
        self.save_depth_png_enabled = bool(args.save_depth_png and self.event_log_enabled)
        self.frame_index = 0
        ...
        if self.event_log_enabled and not self.event_log.handlers:
        ...
        if self.control_topic:
            ...
            self.command_pub = self.create_publisher(
                MultiDOFJointTrajectoryPoint, "/scalenav/trajectory_point", 10
            )
        self.path_pub = self.create_publisher(PathMsg, "/scalenav/planned_path", 10)
        self.visual_odom_pub = self.create_publisher(
            Odometry, "/scalenav/odom", 20
        )
        self.flight_pub = self.create_publisher(MarkerArray, "/scalenav/flight", 1)
        ...
        self.goal_visual_pub = self.create_publisher(
            PoseStamped, "/scalenav/goal", goal_qos
        )
    ...
    def update_graph_visualization(
        ...
        for name, path, rgba, scale in (
            ...
            marker_id += 1
            marker.ns = f"scalenav_graph/{name}"
        ...
        for label, point, rgba, marker_type, scale in (
            ...
            marker_id += 1
            marker.ns = f"scalenav_graph/{label}"
    ...
    def publish_control(self) -> None:
        ...
        if self.colosseum_command_pub is not None:
            ...
            published_topic = "/scalenav/trajectory_point"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--disable-event-log", action="store_true",
        help="Disable planner JSONL, depth PNG, and flight-statistics files.",
    )
    parser.add_argument(
    ...
    parser.add_argument("--graph-marker-topic", default="/scalenav/graph_markers")
