...
class LIOInterface {
 public:
  ...
  // ScaleNav uses a rolling local map.  The boundary below is still kept as
  // the coordinate origin for region/grid indexing, but it must not become a
  // mission-sized hard wall when rolling mapping is enabled.  Keep the
  // bounded behaviour as the default for the standalone LIO contract/tests.
  ...
  void pruneLocked(const Eigen::Vector3f &pose) {
    ...
    for (const auto &point : cloud_->points) {
      const Eigen::Vector3f delta(
        point.x - pose.x(), point.y - pose.y(), point.z - pose.z());
