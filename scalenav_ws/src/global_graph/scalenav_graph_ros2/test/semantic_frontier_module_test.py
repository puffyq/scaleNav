...
def json_safe(value: Any) -> Any:
    """Convert diagnostic infinities to JSON null for browser replay."""
...
def convert_image_bytes(path: Path, kind: str) -> dict[str, Any]:
    """Compress a session image so the HTML report stays browser-friendly."""
    if kind == "rgb":
        resize = "640x360>"
        args = ["convert", str(path), "-resize", resize, "-quality", "82", "jpeg:-"]
        mime = "image/jpeg"
    elif kind == "semantic":
        resize = "640x360>"
        args = ["convert", str(path), "-resize", resize, "png:-"]
        mime = "image/png"
    else:
        resize = "320x180>"
        args = ["convert", str(path), "-resize", resize, "png:-"]
    ...
    return {"data": base64.b64encode(proc.stdout).decode("ascii"), "mime": mime}
...
def render_html(result: dict[str, Any]) -> str:
    payload = json.dumps(json_safe(result), separators=(",", ":"), allow_nan=False)
    return """<!doctype html>
...
def build_log_result(session: Path) -> dict[str, Any]:
    ...
    for index, semantic_record in enumerate(semantic_records):
        ...
        middle_scores = [float(v) for v in means[PATCH_ROWS // 2]
                         if v is not None and math.isfinite(float(v))]
        frame_baseline = (sorted(middle_scores)[int(0.25 * (len(middle_scores) - 1))]
                          if middle_scores else 0.0)
        calibrated_middle = [
            max(0.0, min(1.0, (float(v) - min(frame_baseline, 0.25)) /
                             max(1.0e-3, 1.0 - min(frame_baseline, 0.25))))
            if v is not None else None for v in means[PATCH_ROWS // 2]]
        ...
        frames.append({
            ...
            "rgb": ({**convert_image_bytes(session / rgb_record["file"], "rgb"), "kind": "rgb"}
                    if rgb_record and rgb_record.get("file") else None),
            "depth": ({**convert_image_bytes(session / depth_record["file"], "depth"), "kind": "depth"}
                      if depth_record and depth_record.get("file") else None),
            "heatmap": {**convert_image_bytes(session / semantic_record["file"], "semantic"),
                        "kind": "heatmap"},
...
def main() -> int:
    ...
    parser.add_argument("--max-frames", type=int, default=0,
                        help="limit real-log frames in the browser report (0 = all)")
    args = parser.parse_args()
    result = build_log_result(args.session) if args.session else simulate()
    if args.session and args.max_frames > 0:
        result["frames"] = result.get("frames", [])[:args.max_frames]
