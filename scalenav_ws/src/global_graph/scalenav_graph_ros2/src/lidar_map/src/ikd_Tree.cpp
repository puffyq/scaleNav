...
KD_TREE<PointType>::KD_TREE(float delete_param, float balance_param, float box_length,
                            bool enable_multithread_rebuild) {
    delete_criterion_param = delete_param;
    balance_criterion_param = balance_param;
    downsample_size = box_length;
    multithread_rebuild_enabled_ = enable_multithread_rebuild;
    Rebuild_Logger.clear();           
    termination_flag = false;
    if (multithread_rebuild_enabled_) start_thread();
}

template <typename PointType>
KD_TREE<PointType>::~KD_TREE()
{
    if (multithread_rebuild_enabled_) stop_thread();
