...
void TopoGraph::generateBubble(const Eigen::Vector3f &low_bd,
  ...
  // supplementCubeBubbleGeneration recursively visits the boxes around a
  // Bubble. In planar mode the center is projected back to planar_z_, so a
  // Z-only child can reproduce its parent's X/Y footprint indefinitely.
  // The logged forest frame hit exactly that branch and overflowed the OpenMP
  // worker stack. Require geometric progress in the dimensions used by the
  // topology before entering another recursive level.
  using Bounds = std::pair<Eigen::Vector3f, Eigen::Vector3f>;
  static thread_local std::vector<Bounds> recursion_bounds;
  if (!recursion_bounds.empty()) {
    constexpr float kProgressEpsilon = 1e-4F;
    const Eigen::Vector3f parent_size =
      recursion_bounds.back().second - recursion_bounds.back().first;
    const Eigen::Vector3f child_size = high_bd - low_bd;
    const bool progressed = planar_graph_ ?
      (child_size.x() < parent_size.x() - kProgressEpsilon ||
       child_size.y() < parent_size.y() - kProgressEpsilon) :
      (child_size.array() < parent_size.array() - kProgressEpsilon).any();
    if (!progressed) return;
  }
  recursion_bounds.emplace_back(low_bd, high_bd);
  struct RecursionGuard {
    std::vector<Bounds> &bounds;
    ~RecursionGuard() { bounds.pop_back(); }
  } recursion_guard{recursion_bounds};
