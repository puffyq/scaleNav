...
EPIC_GRAPH_LOG_FILE="${EPIC_GRAPH_LOG_FILE:-$PROJECT_ROOT/log_event/epic_graph_snapshots.jsonl}"
...
EPIC_DIAGNOSTIC_LOG_PERIOD_MS="${EPIC_DIAGNOSTIC_LOG_PERIOD_MS:-2000}"
EPIC_SKELETON_REBUILD_PERIOD_MS="${EPIC_SKELETON_REBUILD_PERIOD_MS:-200.0}"
...
EPIC_ROUTE_PLAN_PERIOD_MS="${EPIC_ROUTE_PLAN_PERIOD_MS:-100}"
EPIC_LOCAL_GOAL_RESERVE_M="${EPIC_LOCAL_GOAL_RESERVE_M:-0.0}"
...
EPIC_SEMANTIC_COST_WEIGHT="${EPIC_SEMANTIC_COST_WEIGHT:-0.0}"
EPIC_SEMANTIC_NODE_EMA_ALPHA="${EPIC_SEMANTIC_NODE_EMA_ALPHA:-0.3}"
EPIC_SEMANTIC_ROUTE_REPLAN_ENABLED="${EPIC_SEMANTIC_ROUTE_REPLAN_ENABLED:-false}"
EPIC_SEMANTIC_VISUALIZATION_MAX_SCORE="${EPIC_SEMANTIC_VISUALIZATION_MAX_SCORE:-0.4}"
EPIC_SEMANTIC_BASELINE_QUANTILE="${EPIC_SEMANTIC_BASELINE_QUANTILE:-0.25}"
...
EPIC_SPECULATIVE_ENABLED="${EPIC_SPECULATIVE_ENABLED:-false}"
...
if [[ "$EPIC_ONLINE" == "1" ]]; then
  ...
  ros2 launch openseek_epic_ros2 epic_graph.launch.py \
    ...
    "graph_log_file:=$EPIC_GRAPH_LOG_FILE" \
    ...
    "diagnostic_log_period_ms:=$EPIC_DIAGNOSTIC_LOG_PERIOD_MS" \
    ...
    "local_graph_radius_m:=${EPIC_LOCAL_GRAPH_RADIUS_M:-35.0}" \
    ...
    "semantic_route_replan_enabled:=$EPIC_SEMANTIC_ROUTE_REPLAN_ENABLED" \
    "semantic_visualization_max_score:=$EPIC_SEMANTIC_VISUALIZATION_MAX_SCORE" \
    "semantic_baseline_quantile:=$EPIC_SEMANTIC_BASELINE_QUANTILE" \
  ...
  epic_pid=$!
  echo "EPIC 在线链路已启动: global=$EPIC_GLOBAL_GOAL_TOPIC next=$EPIC_NEXT_GOAL_TOPIC update=${EPIC_UPDATE_PERIOD_MS}ms rebuild=${EPIC_SKELETON_REBUILD_PERIOD_MS}ms log=${EPIC_DIAGNOSTIC_LOG_PERIOD_MS}ms"
