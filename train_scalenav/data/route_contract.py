...
class RouteQualityGate:
    ...
    def evaluate(
        ...
        *,
        ...
        if np.any(distances > radii[:-1] + radii[1:] + 1.0e-5):
        ...
        if flags != RouteQualityFlag.NONE:
            weight = 0.0
        return RouteQualityResult(flags, path_length, minimum_clearance, curvature, float(weight))
...
def sample_route_bubbles(
    ...
    selected = sorted(
        sorted(deduplicated, key=lambda item: item[1], reverse=True)[:valid_slots],
...
class RouteTable:
    ...
    def validate(self, frame_count: int | None = None) -> None:
        ...
        if version != ROUTE_DATASET_VERSION:
...
def pack_route_records(records: Iterable[RouteRecord]) -> RouteTable:
    ...
    for record in records:
        ...
        path_lengths.append(polyline_arclength(points)[1] if len(points) else 0.0)
