...
def test_outer_lattice_rows_cannot_reach_a_level_route_without_projection():
    transform = StateTransform()
    primitive = transform.lattice_primitive
    _, pitch = primitive.getAngleLattice()
    minimum_outer_pitch = torch.abs(pitch[[0, -1]]) - primitive.pitch_diff
    assert torch.all(minimum_outer_pitch > 0.0)
    np.testing.assert_allclose(
