...
without modification or retraining.  In the latest ten-trial GCN run, all
trials reached the goal with no collision or timeout; successful flights
averaged $171.59$~m at $5.11$~m/s.  These results make the central design
boundary explicit: graph learning proposes a route direction, while
geometric topology and collision checks retain authority over execution.
\end{abstract}
...
\vspace{-2pt}
\caption{Topology-first ScaleNav architecture.  Depth and odometry maintain the
persistent graph of verified bubbles and witness edges, while RGB and a text
query provide semantic route preference.  The same graph feeds a five-way GCN
proposal; reachable search and collision checks accept the witness route before
the unchanged local executor receives a goal.  Semantics suggests a route;
geometry retains safety authority.}
...
original batch contains 27 valid trials and the latest GCN batch contains
10 valid trials.  Startup failures and one manually interrupted trial are excluded
...
It is not a trajectory plot of the ten-trial GCN batch; the quantitative GCN
...
figure is not the ten-trial GCN batch reported in
...
different sample counts ($n=27$ and $n=10$); continuous entries are means over
...
GCN-ScaleNav ($n=10$) & 100.0 & 0.0 & 0.0 & 171.59 & 33.61 & 5.11 & 6.25 \\
\bottomrule
\end{tabular}
}
\end{table}

Table~\ref{tab:single_run_validation} reports the latest ten-trial GCN batch
alongside the earlier original-planner batch.  The GCN batch completed all
trials, but its mean path and flight time are lower than the original batch's
despite the different sample counts and seeds.  These differences are
descriptive only and do not establish a causal improvement.

The latest GCN run produced no collision or timeout events: all ten trials
reached the goal and satisfied the final position and settling-speed
tolerances.  Failure-mechanism analysis from the earlier 60-trial run is not
used to characterize this updated batch.

To expose the effect of adding the graph route layer across executors, we
report the complete conventional flight metrics for same-scene ten-trial Map2
batches.  The graph-enabled EGO batch uses the 600-s mission timeout with a
60-s no-progress termination; the latest graph-enabled SUPER batch uses the
90-s mission timeout with no separate no-progress termination.

\begin{table*}[t]
\caption{Ten-Trial Map2 Closed-Loop Comparison Using Conventional Flight
Metrics.  The first row of each executor pair is the baseline without the
ScaleNav graph; the second row uses the graph route layer.}
...
Method & Success (\%) & Collision (\%) & Timeout (\%) & Path (m) & Time (s) & Avg. speed (m/s) & Max. speed (m/s) \\
\midrule
YOPO-Simple (no graph) & 0 & 100 & 0 & $129.79\pm0.58$ & $61.46\pm0.36$ & $2.11\pm0.01$ & $6.46\pm0.02$ \\
ScaleNav (YOPO) & 100 & 0 & 0 & $196.30\pm13.36$ & $37.48\pm2.50$ & $5.24\pm0.09$ & $6.42\pm0.05$ \\
EGO-Planner (no graph) & 0 & 100 & 0 & $73.24\pm0.69$ & $54.32\pm1.05$ & $1.35\pm0.02$ & $2.43\pm0.25$ \\
ScaleNav+EGO & 0 & 50 & 50 & $54.50\pm32.30$ & $72.22\pm37.68$ & $0.85\pm0.40$ & $2.18\pm0.27$ \\
SUPER (no graph) & 0 & 100 & 0 & $47.07\pm28.27$ & $10.25\pm4.99$ & $4.30\pm0.68$ & $5.60\pm0.23$ \\
ScaleNav+SUPER & 80 & 10 & 10 & $180.36\pm16.09$ & $42.60\pm4.09$ & $4.24\pm0.22$ & $5.15\pm0.06$ \\
\bottomrule
\end{tabular}
}
\end{table*}

Outcome rates use all ten valid trials.  For a condition with at least one
success, path, time, and speed are mean $\pm$ sample standard deviation over
successful flights; when no trial succeeds, they are the corresponding
statistics observed before collision or timeout and are not completion
metrics.  The latest ScaleNav+SUPER batch completes 8/10 trials, so its
continuous entries use the eight successful flights.  The graph-enabled conditions use the semantic route
layer with the same ``blocks, wall'' query and the corresponding local
executor.
...
The latest GCN summary contains ten recorded, valid navigation trials; all
ten succeeded.  The original ScaleNav summary contains 27 valid trials and
one interruption.  A startup failure is not counted as a navigation failure.
Since the valid counts are different and the trials are not seed-matched, the
closed-loop comparison is descriptive rather than a causal significance test.
Equal-sized, seed-matched trials remain necessary for a definitive comparison.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
latest ten-trial evaluation is collision-free, but the unmatched sample size
does not support a general safety claim.  Future work must add
reachable-clearance and braking-aware features, mask unsafe columns before
learned ranking, and rerun the two conditions with matched valid trial counts.
