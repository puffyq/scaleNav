...
\vspace{-2pt}
\caption{Integrated cross-executor evaluation.  The first three columns overlay
each standalone executor (blue, orange, or green) with its TopoGuide counterpart
(red) on the same obstacle backdrop; the fourth column adds FAR Planner
(purple).  The lower row uses shared normalized mission-time and wall-time
axes, with identical ranges and ticks across all four panels:
TopoGuide columns report incremental route-layer timing, while FAR reports its
V-Graph update time and path-search time from the planner
console trace.  Curves are binned medians; FAR's untimestamped resource
messages are placed at the midpoint of adjacent timestamped planner messages
within the recorded mission interval.  FAR success is based on two trials.
Stars mark completed goals and crosses mark incomplete baseline flights.}
...
run.  The fourth column adds the FAR trajectory and its console-derived
resource profile; these module timings are not a matched total-compute comparison.
...
\emph{Global-planning references.}  As supplementary references on the same
...
at P95).  The first three lower panels of Fig.~\ref{fig:integrated_results} separate
...
and compare the standalone executor with the TopoGuide-guided stack at
...
\begin{figure}[t]
\centering
\safefig[width=0.76\columnwidth]{pics/experiments/scale_sweep.pdf}
...
Matched query & 100 & 0 & 0 & $171.6\pm6.6$ & $33.6\pm1.3$ \\
...
GCN & 100.0 & 0.0 & 0.0 & $171.6\pm6.6$ & $33.6\pm1.3$ \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{figure}[!t]
\centering
\safefig[width=0.80\columnwidth]{pics/candidates/pearl_powerline_graph3d_candidate_v26.png}
