...
A* downstream.  (b)~A recorded graph state is paired with its logged geometry,
then an offline A* search on the privileged static map produces the direction
target ($c=2$, the central column) for that training sample.  The privileged
map and target are used only during training and are unavailable at deployment.}
\label{fig:gcn_model}
\end{figure}
