...
Each closed-loop condition contains 10 paired trials with identical initial
states, goals, scenes, and random seeds across methods.  We report the mean,
...
The ablation removes one route-level component at a time while retaining the
same local policy, graph geometry, start, goal, and ten-trial seed set.  The
full system is compared with a no-target condition containing no
task-relevant semantic target, and variants without persistent far-field
nodes, world-frame route memory, or the ordered witness corridor.  In the
no-target condition, PEARL remains active but the ``tree'' query has no
corresponding object; it therefore tests operation without
task-relevant semantic guidance rather than semantic-front-end runtime.  The
last variant uses the unconstrained YOPO primitive generator with the same
frontier goal.  Unmeasured matched conditions remain reserved below.

\begin{table}[t]
\caption{Component Ablation over Ten Paired Trials. SE: semantic front end;
...
\toprule
& \multicolumn{4}{c}{Component} & \multicolumn{6}{c}{Closed-Loop Performance} \\
\cmidrule(lr){2-5} \cmidrule(lr){6-11}
Variant & SE & FF & RM & WC & Succ. (\%)$\uparrow$ & Coll. (\%)$\downarrow$ & T/O (\%)$\downarrow$ & Path (m) & $\bar v$ (m/s) & $v_{\max}$ (m/s) \\
\midrule
\textbf{Full ScaleNav} & \checkmark & \checkmark & \checkmark & \checkmark & 100 & 0 & 0 & 154.311$\pm$2.887 & 4.982$\pm$0.081 & 5.893$\pm$0.118 \\
\midrule
w/o semantic front end  & $\times$   & $\times$   & \checkmark & \checkmark & 100 & 0 & 0 & 154.815$\pm$4.042 & 2.841$\pm$0.091 & 4.571$\pm$0.220 \\
No task-relevant semantics & \checkmark & \checkmark & \checkmark & \checkmark & 80 & 20 & 0 & 127.680$\pm$55.997 & 3.110$\pm$0.203 & 4.564$\pm$0.316 \\
w/o far-field nodes     & \checkmark & $\times$   & \checkmark & \checkmark & \blank & \blank & \blank & \blank & \blank & \blank \\
w/o route memory        & \checkmark & \checkmark & $\times$   & \checkmark & \blank & \blank & \blank & \blank & \blank & \blank \\
w/o witness corridor    & \checkmark & \checkmark & \checkmark & $\times$   & \blank & \blank & \blank & \blank & \blank & \blank \\
\bottomrule
\end{tabular}
}
\end{table}
