...
A* downstream.  (b)~A synchronized RGB--depth frame and its logged online
skeleton are paired with the privileged expert direction label ($c=2$, the
central column) generated by the 35-m A* look-ahead.  The privileged label is
used only during training and is unavailable at deployment.}
\label{fig:gcn_model}
\end{figure}
