...
We organize the evaluation into three groups.  Experiment A is the main
...
same local-policy architecture.  Experiment C changes only the open-vocabulary
query (e.g., ``avoid buildings,'' ``avoid trees,'' and ``avoid power lines'')
and measures the resulting route changes without retraining the local policy.
Runtime and memory are reported separately for every group.
...
The main comparison evaluates the complete ScaleNav system against the
following published planners:
\begin{itemize}
...
Internal variants are reported only in the component ablation.  It uses four
matched stages: ``YOPO'' receives only the frontier goal; ``Rolling-Goal YOPO''
adds the persistent graph and its rolling waypoint; ``Corridor-Conditioned
YOPO'' additionally receives the ordered witness corridor; and ``ScaleNav''
adds the open-vocabulary risk field.  Semantic weights are zero in the two
geometric variants.  These stages isolate route memory, corridor conditioning,
and semantic foresight, respectively.  All four use the same primitive
generator and local state.  The semantic front end is PEARL; a Qwen3-VL adapter
can be substituted through the same observation contract.
...
that combines forests and buildings without mode markers.  The open-vocabulary
trials use matched scenes with queries for buildings, trees, and power lines,
together with hard-negative queries.
...
SUPER             & \blank & \blank & \blank & \blank & \blank \\
...
The ablation incrementally adds route-level memory, corridor conditioning, and
open-vocabulary route ranking to the local baseline.
Table~\ref{tab:semantic_results} reports branch selection, route stability, and
semantic exposure.
...
Rolling-Goal YOPO    & \blank & \blank & \blank & \blank \\
