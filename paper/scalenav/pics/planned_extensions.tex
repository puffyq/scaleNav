...
\node[title] at (88,68) {Narrow Planner Interface and Replaceable Semantics};
\node[subtitle] at (88,63) {Keep topology and semantic reasoning upstream of goal-conditioned local planners};

% Current moving-goal interface
\filldraw[panel,fill=orangebg,draw=orange] (0,8) rectangle (85,58);
\node[group,anchor=west] at (4,54) {A. Moving-goal planner interface};
\node[planned,anchor=east] at (81,54) {CURRENT};
...
\node[nodetitle] at (42,44.5) {Arc-length lookahead};
\draw[orange,line width=1.2pt] (34,38)--(38,42)--(43,39)--(50,45);
\fill[orange] (47.8,42.8) circle (1.2mm);

\filldraw[box,fill=blue!7,draw=blue!60] (61,35) rectangle (81,48);
\node[nodetitle] at (71,44.5) {Moving local goal};
\node[note] at (71,39.5) {PoseStamped};
...
\node[nodetitle] at (21.5,25.5) {Goal-conditioned planner};
\node[note] at (21.5,21) {YOPO / EGO / SUPER};
\filldraw[box,fill=white,draw=green!60] (49,17) rectangle (76,29);
\node[nodetitle] at (62.5,25.5) {Depth remains authoritative};
\node[note] at (62.5,21) {short-horizon execution};
\draw[arrow,draw=blue] (71,35)--(71,29);
\draw[arrow,draw=green] (49,23)--(34,23);
\node[note,text=orange] at (42.5,12) {executor boundary: one world-frame moving goal};
