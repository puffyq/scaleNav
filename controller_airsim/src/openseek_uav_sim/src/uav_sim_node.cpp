...
class OpenSeekUavSim final : public rclcpp::Node
{
public:
  OpenSeekUavSim()
  : Node("openseek_uav_sim"), tf_broadcaster_(*this)
  {
    ...
    pose_cmd_sub_ = create_subscription<geometry_msgs::msg::PoseStamped>(
      "/openseek/position_cmd", 10,
      std::bind(&OpenSeekUavSim::pose_command, this, std::placeholders::_1));
    trajectory_cmd_sub_ =
      create_subscription<trajectory_msgs::msg::MultiDOFJointTrajectoryPoint>(
      "/openseek/trajectory_point", 10,
      std::bind(&OpenSeekUavSim::trajectory_command, this, std::placeholders::_1));
    collision_sub_ = create_subscription<std_msgs::msg::Bool>(
      "/sim/collision", 10,
      std::bind(&OpenSeekUavSim::emergency_stop, this, std::placeholders::_1));
    emergency_sub_ = create_subscription<std_msgs::msg::Bool>(
      "/openseek/emergency_stop", 10,
      std::bind(&OpenSeekUavSim::emergency_stop, this, std::placeholders::_1));
    reset_service_ = create_service<std_srvs::srv::Trigger>(
      "/openseek/reset_sim",
      std::bind(
        &OpenSeekUavSim::reset_service, this, std::placeholders::_1,
        std::placeholders::_2));
    ...
    simulation_timer_ = create_wall_timer(
      ...
      std::bind(&OpenSeekUavSim::simulation_step, this));

    RCLCPP_INFO(
      get_logger(),
      "OpenSeek UAV simulator ready: command=/openseek/trajectory_point, odom=/sim/odom");
  }
...
private:
  ...
  void emergency_stop(const std_msgs::msg::Bool::SharedPtr message)
  {
    ...
    RCLCPP_ERROR(get_logger(), "Emergency stop latched; call /openseek/reset_sim to reset");
  }
    ...
    std_srvs::srv::Trigger::Response::SharedPtr response)
  {
    ...
    response->message = "OpenSeek UAV simulator reset";
  }
...
int main(int argc, char ** argv)
{
  ...
  rclcpp::spin(std::make_shared<OpenSeekUavSim>());
  rclcpp::shutdown();
  return 0;
}
