...
TEST(EpicIntegration, TenHertzDepthAndFreeRayUpdatesPreserveTheMapContract)
{
  LIOInterface map;
  ...
  EXPECT_GE(free_ray_updates, 1);
  EXPECT_EQ(map.accumulatedCloudSnapshot().size(), 1U);
  EXPECT_GT(map.freeSpaceSnapshot().size(), 20U);
  // The endpoint remains occupied while the separate side ray remains free.
